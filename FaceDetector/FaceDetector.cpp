...
Detector::Detector(const std::string &models_path, bool retinaface):
        _nms(0.4),
        _threshold(0.6),
        _mean_val{104.f, 117.f, 123.f},
        _retinaface(retinaface),
...
void Detector::Init(const std::string &model_path) {
    int ret = Net->load_param((model_path + "/face.param").c_str());
    ret = Net->load_model((model_path + "/face.bin").c_str());
}
...
//    timer.toc("anchor:");
    ...
    for (int i = 0; i < anchor.size(); ++i)
    {
        if (*(ptr1+1) > _threshold)
        {
            ...
            std::cout << tmp1.cx << ", " << tmp1.cy << ", " << tmp1.sx << ", " << tmp1.sy << std::endl;
...
//    printf("%d\n", (int)total_box.size());
    ...
    {
        cv::Rect rect(total_box[j].x1, total_box[j].y1, total_box[j].x2 - total_box[j].x1, total_box[j].y2 - total_box[j].y1);
        rect = rectCenterScale(rect, 0.25);
        total_box[j].x1 = rect.x;
        total_box[j].y1 = rect.y;
        total_box[j].x2 = rect.x + rect.width;
        total_box[j].y2 = rect.y + rect.height;
