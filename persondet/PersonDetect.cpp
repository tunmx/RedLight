...
PersonDetect::PersonDetect(const std::string &models_path) {
    initialized_ = false;
    Init(models_path);
}
...
int PersonDetect::Init(const std::string &models_path) {
    std::cout << "start init person detect interpreter." << std::endl;
    std::string model_file = models_path + "/ssd_quant.mnn";
...
int PersonDetect::detect(const cv::Mat &img_src, std::vector<ObjectInfo> &objects) {
    if (!initialized_) {
